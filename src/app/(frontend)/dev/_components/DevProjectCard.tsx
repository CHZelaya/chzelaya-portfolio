...
export function DevProjectCard({ project }: DevProjectCardProps) {
    ...
    const statusClass =
        project.status === 'frozen'
            ? 'text-(--color-accent) border border-(--color-accent-border) bg-(--color-accent-dim)'
            : project.status === 'in-progress'
                ? 'text-amber-400 border border-amber-400/25 bg-amber-400/6'
                : project.status === 'live'
    ...
    return (
        <div className="bg-(--color-bg-raise) border border-(--color-border-mid) rounded-sm overflow-hidden transition-[border-color,transform] duration-200 hover:border-white/20 hover:-translate-y-0.5">
            ...
            {/* Cover */}
            <div className="relative w-full h-45 bg-(--color-bg-subtle) overflow-hidden">
                {project.coverImage ? (
                    <Image
                ...
                {project.scribbleNote && (
                    <span className="absolute bottom-3 left-3 font-hand text-[1.05rem] text-(--color-accent)/85 -rotate-2 pointer-events-none leading-snug">
                        {project.scribbleNote}
                    </span>
                )}
            </div>
            ...
            <div className="p-4 md:p-5">
                ...
                {slug ? (
                    <a
                        href={`/projects/${slug}`}
                        className="text-[10px] tracking-[0.14em] uppercase text-(--color-accent) border-b border-(--color-accent-border) pb-0.5 hover:border-(--color-accent) transition-colors font-mono"
                    >
                        Read case study →
                    </a>
                ) : (
                    <span className="text-[10px] tracking-[0.14em] uppercase text-(--color-text-dim) border-b border-(--color-border) pb-0.5 font-mono pointer-events-none">
                        Case study coming →
                    </span>
                )}
            </div>
